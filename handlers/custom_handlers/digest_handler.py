...
<i>💥 Новости обещают быть интересными! 🌅</i>""",
    ...
    if news_pack:
        ...
        # ✅ ИСПРАВЛЕНО: update_streak ДО save()
        user_progress.last_pack = next_pack
        user_progress.updated_at = datetime.now()
        streak_grew = update_streak(user_id)  # ✅ ДО save()
        user_progress.save()  # ✅ Сохраняем всё сразу

        bot.send_message(user_id,
                         "<b>/digest</b> → следующая пачка новостей!",
                         parse_mode='HTML')

        # ✅ УРОВЕНЬ и ПРОГРЕСС (тот же user_progress объект)
